(function($) {
	...
	var Topbar = this.Topbar = Backbone.View.extend({
		...
		initialize: function() {
			this.$el.html('<img class="logo" src="' + Tools.resourcePrefix + '<img src="http://i.imgur.com/dsIgSkH.png" alt="Pok&eacute;mon Showdown! (beta)" /><div class="maintabbarbottom"></div><div class="tabbar maintabbar"><div class="inner"></div></div><div class="userbar"></div>');
	...
	var OptionsPopup = this.OptionsPopup = Popup.extend({
		...
		update: function() {
			...
			buf += '<hr />';
			buf += '<p><label class="optlabel">Background: <select name="bg"><option value="">Parukia</option><option value="#344b6c url("http://i.imgur.com/7NCaYhr.jpg") no-repeat left center fixed">Darkrai</option><option value="#546bac url(/fx/client-bg-2.jpg) no-repeat left center fixed">Waterfall</option><option value="#546bac url(/fx/client-bg-ocean.jpg) no-repeat left center fixed">Ocean</option><option value="#344b6c">Solid blue</option>'+(Tools.prefs('bg')?'<option value="" selected></option>':'')+'</select></label></p>';
